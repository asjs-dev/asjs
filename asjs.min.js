...
function singleton(scope,c,i){property(scope,"instance",{get:function(){if(!c.$)c.$=new i();return c.$;}});};
...
var stage;var ASJS={};ASJS.inited;ASJS.start=function(baseClass){if(ASJS.inited)return;ASJS.inited=true;$(document).ready(function(){stage=new ASJS.Stage().instance;stage.init();new baseClass();});};ASJS.Point=function(x,y){var that={};that.x=x||0;that.y=y||0;return that;};ASJS.Rectangle=function(x,y,width,height){var that=new ASJS.Point(x,y);that.width=width||0;that.height=height||0;return that;};ASJS.Matrix=function(a,b,c,d,e,f){var that={};that.a=a||1;that.b=b||0;that.c=c||0;that.d=d||1;that.e=e||0;that.f=f||0;that.translate=function(tx,ty){matrix.e=tx;matrix.f=ty;};
...
(function(){_requestAnimationFrame=that.el.requestAnimationFrame||that.el.mozRequestAnimationFrame||that.el.webkitRequestAnimationFrame||that.el.msRequestAnimationFrame||function(f){return setTimeout(f,1)};})();return that;});};ASJS.EventDispatcher=function(tag){var that={};that.jQuery=$(tag||"<div />");property(that,"el",{get:function(){return that.jQuery[0];}});that.dispatchEvent=function(type,data,bubble){var eventBubble=bubble==undefined?true:bubble;if(eventBubble)that.jQuery.trigger(type,data);else that.jQuery.triggerHandler(type,data);};
that.addEventListener=function(type,callback){if(type==ASJS.MouseEvent.SCROLL&&that.jQuery==new ASJS.Window().instance.jQuery)that.jQuery.scroll(callback);else that.jQuery.on(type,callback);};
...
ASJS.LoaderEvent.LOAD="load";ASJS.LoaderEvent.ERROR="error";ASJS.LoaderEvent.ABORT="abort";ASJS.LoaderEvent.LOAD_START="loadstart";ASJS.LoaderEvent.PROGRESS="progress";ASJS.LoaderEvent.LOAD_END="loadend";ASJS.WindowEvent=function(type){var that=new ASJS.Event(type);return that;};
ASJS.WindowEvent.POPSTATE="popstate";ASJS.WindowEvent.RESIZE="resize";ASJS.WindowEvent.UNLOAD="unload";ASJS.Keyboard=function(){var that={};var _window=new ASJS.Window().instance;var _pressedKeys={};var _downCallback;var _upCallback;that.isPressed=function(which){return _pressedKeys[which];};
...
return size;};ASJS.List=function(){var that=new ASJS.Sprite();var _name;property(that,"val",{get:function(){var value;var i=-1;var l=that.numChildren;while(++i<l){var item=that.getChildAt(i);if(item.checked)value=item.val;};
return value;},set:function(value){if(that.val==value)return;var i=-1;var l=that.numChildren;while(++i<l){var item=that.getChildAt(i);item.checked=item.val==value;}}});property(that,"name",{get:function(){return _name;},set:function(value){_name=value;var i=-1;var l=that.numChildren;while(++i<l){var item=that.getChildAt(i);item.name=_name;}}});that.clearList=function(){while(that.numChildren>0)that.removeChildAt(0);};
that.setListItems=function(items){that.clearList();var i=-1;var l=items.length;while(++i<l)that.addItem(items[i]);};
that.addItem=function(item){item.name=_name;that.addChild(item);};
return that;};ASJS.FormElement=function(tag){var that=new ASJS.Sprite(tag);property(that,"name",{get:function(){return that.getAttr("name");},set:function(value){that.setAttr("name",value);}});that.drawNow=function(){};(function(){that.tabIndex="auto";})();return that;};ASJS.ListItem=function(tag){var that=new ASJS.FormElement(tag);var _checked=false;var _value="";property(that,"name",{get:function(){return that.getAttr("name");},set:function(value){that.setAttr("name",value);}});property(that,"checked",{get:function(){return _checked;},set:function(value){_checked=value;}});property(that,"val",{get:function(){return _value;},set:function(value){_value=value;}});return that;};ASJS.Cell=function(tag){var that=new ASJS.Sprite(tag);var _data={};var _checked=false;property(that,"name",{get:function(){return that.getAttr("name");},set:function(value){that.setAttr("name",value);}});property(that,"checked",{get:function(){return _checked;},set:function(value){_checked=value;}});property(that,"data",{get:function(){return _data;},set:function(value){_data=value;if(_data.id)that.id=_data.id;that.showData();}});that.drawNow=function(){};
...
function initView(){that.setCSS("position","relative");that.mouseChildren=false;that.addEventListener(ASJS.MouseEvent.CLICK,onClick);};
(function(){initView();})();return that;};
...
function init(){that._fileInput.setAttr("type","file");that._fileInput.addEventListener(ASJS.Event.CHANGE,that._onChange);that._fileInput.visible=false;that.addChild(that._fileInput);_preview.setSize("100%","100%");_preview.move(0,0);that.addChild(_preview);that.addEventListener(ASJS.MouseEvent.CLICK,onClick);};
(function(){init();})();return that;};
...
function init(){that._fileInput.removeEventListeners();that._fileInput.addEventListener(ASJS.Event.CHANGE,that._onChange);_reader.onload=readerOnLoad;};
(function(){init();})();return that;};
...
ASJS.TextInput.TYPE_TEXT="text";ASJS.TextInput.TYPE_PASSWORD="password";ASJS.TextInput.TYPE_EMAIL="email";ASJS.TextInput.TYPE_NUMBER="number";ASJS.RadioButton=function(){var that=new ASJS.FormElement("<label />");var _super={};var _radio=new ASJS.DisplayObject("<input />");var _label=new ASJS.DisplayObject();extendProperty(_super,that,"enabled");property(that,"enabled",{set:function(value){_super.enabled=value;_radio.enabled=that.enabled;that.drawNow();}});property(that,"radio",{get:function(){return _radio;}});property(that,"name",{get:function(){return _radio.getAttr("name");},set:function(value){_radio.setAttr("name",value);}});property(that,"checked",{get:function(){return _radio.jQuery.is(":checked");},set:function(value){_radio.jQuery.prop("checked",value);if(value)_radio.jQuery.change();}});property(that,"val",{get:function(){return _radio.getAttr("value");},set:function(value){_radio.setAttr("value",value);}});that.drawNow=function(){};function init(){_radio.setAttr("type","radio");_radio.visible=false;that.addChild(_radio);_label.setSize("100%","100%");_label.enabled=false;that.addChild(_label);};
(function(){init();})();return that;};ASJS.AbstractFilter=function(value){var that={};that.value=value;that.execute=function(){};
